...
import { FormularioCompra, type ChipCompra } from "./FormularioCompra";
import {
  IconeX,
...
type Passo = "escolher" | "manual" | "lista";

// Registrar Compra (ADR-005/ADR-017). Tela cheia sobre o shell: passo "como
// registrar" → formulário de Compra. A via "Manual" parte do zero; a via
// "Marcar da lista" pré-preenche os chips com os Itens ativos da Lista.
export function RegistrarConteudo({
  itensDaLista,
}: {
  itensDaLista: ChipCompra[];
}) {
  const router = useRouter();
  const [passo, setPasso] = useState<Passo>("escolher");
  const temLista = itensDaLista.length > 0;
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-md flex-1 flex-col md:max-w-3xl">
        ...
        {passo === "escolher" && (
          <div className="px-5.5 pt-3.5">
            ...
            {/* "Marcar da lista" (ADR-017): só aparece com ≥1 Item ativo na Lista. */}
            {temLista && (
              <button
                type="button"
                onClick={() => setPasso("lista")}
                className="mt-3 flex w-full items-center gap-3.5 rounded-[18px] border-[1.5px] border-borda-forte bg-superficie p-4.5 text-left"
              >
                <span
                  className="flex h-[46px] w-[46px] flex-none items-center justify-center rounded-[14px] text-[22px]"
                  style={{
                ...
                <span className="flex-1">
                  ...
                  <span className="mt-px block text-[13px] text-suave">
                    Confirmar itens que você já comprou
                  </span>
                </span>
                <span className="flex text-[#cfc7bb]">
                  <IconeChevronDireita tamanho={18} />
